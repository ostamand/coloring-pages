...
export async function addNewPage(
    ...
    const { generateScript, prompt, seed, collectionName, generatedOn, tags } =
        generateConfig;
    ...
    const resultPage = await db.queryArray(
        `INSERT INTO pages (coloring_path, colored_path, generate_script, prompt, seed, collection_name, generated_on) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id;`,
        [
            coloringPath,
            coloredPath || null,
            generateScript,
...
export async function getLastPrompts(db: Client, n: number) {
    const result = await db.queryArray(
        `SELECT prompt FROM pages ORDER BY created_on DESC LIMIT $1;`,
        [n],
    );
...
export async function getPages(db: Client, limit: number) {
    ...
    const result = await db.queryObject(
        ...
        SELECT pages.*, page_tags.tags FROM (
