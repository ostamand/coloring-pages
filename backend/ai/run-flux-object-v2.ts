...
import { SYSTEM_INSTRUCTIONS_OBJECT_v1 } from "./instructions/sdxl.ts";
...
async function main(args: string[]) {
    ...
    const instructionsFn = (lastPrompts: string) => {
        return SYSTEM_INSTRUCTIONS_OBJECT_v1 + lastPrompts;
    };
...
// example: deno run --allow-all ai/run-flux-object-v2.ts --config ai/configs/flux-v3.json --out tmp
if (import.meta.main) {
